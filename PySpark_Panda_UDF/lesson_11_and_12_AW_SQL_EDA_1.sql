...
-- MAGIC %md
-- MAGIC
-- MAGIC ### Using Common Table Expressions, here CTE could potentially replace a Subquery, making the sql code cleaner, more readable and maintainable because of the isolated CTE
...
SELECT c.NumberChildrenAtHome as NumberChildrenAtHome , SUM(t.SalesAmount) as SalesAmount
FROM t_salesinfo t
...
-- DBTITLE 1,Enter your answer in the cell below
SELECT c.NumberChildrenAtHome as NumberChildrenAtHome , ROUND(SUM(t.SalesAmount),0) as SalesAmount
FROM t_salesinfo t
INNER JOIN t_customerinfo c
ON (t.CustomerKey = c.CustomerKey)
GROUP BY NumberChildrenAtHome 
...
-- DBTITLE 1,Enter your answer in the cell below
SELECT c.Gender as Genders , t.FiscalYear as FiscalYear, ROUND(AVG(t.SalesAmount)) as SalesAmount
FROM t_salesinfo t
INNER JOIN t_customerinfo c
ON (t.CustomerKey = c.CustomerKey)
GROUP BY Genders,  FiscalYear
ORDER BY Genders,  FiscalYear, SalesAmount DESC
