...
--  Code your answer here.-- If there are spaces in the column header, enclose the label in backticks 
SELECT year(OrderDate), 
       substr(SalesOrderNumber,1,5) as SalesOrderNumber,
        length(SalesOrderNumber) as SalesOrderNumberlength,
       round(SalesAmount) as SalesAmount
FROM factinternetsales 
...
SELECT sum(SalesAmount) as `Total Sales`,
       avg(SalesAmount) as `Avg Sales`,
       sum(SalesAmount) - avg(SalesAmount) as `Sales Over/Under Avg`, 
